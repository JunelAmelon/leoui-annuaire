...
import { Receipt, Plus, Search, Download, Eye, CheckCircle, Clock, AlertCircle, X, Trash2, Edit, Send, Sparkles, ArrowRight } from 'lucide-react';
  ...
  return (
    <PrestataireDashboardLayout>
      <div className="relative max-w-4xl mx-auto space-y-6">
        ...
        {/* Overlay de fonctionnalité à venir */}
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-white/70 backdrop-blur-sm rounded-2xl">
          <div className="max-w-md mx-auto text-center p-8 bg-white rounded-2xl shadow-xl border border-rose-100 animate-in fade-in zoom-in duration-300">
            ...
            <h3 className="text-xl font-serif text-charcoal-900 mb-2" style={{ fontWeight: 400 }}>
              Gestion automatisée des factures
            </h3>
            <p className="text-sm text-charcoal-500 mb-4 leading-relaxed">
              Une expérience de facturation entièrement repensée arrive bientôt. 
              Gérez vos échéances, relances automatiques, et suivi de paiement en temps réel.
            </p>
            <div className="flex items-center justify-center gap-2 text-xs text-rose-600 font-medium">
        ...
        {/* Header */}
        <div className="flex items-center justify-between gap-4 flex-wrap opacity-50 pointer-events-none">
          <div>
          ...
          <button className="flex items-center gap-2 px-4 py-2.5 bg-rose-300 text-white rounded-xl text-sm font-medium cursor-not-allowed">
        ...
        {/* List - floutée */}
        {loading ? (
          <div className="space-y-3 opacity-50 pointer-events-none">{[1,2,3].map(i => <div key={i} className="h-28 bg-white rounded-2xl animate-pulse shadow-sm" />)}</div>
        ) : filtered.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-sm p-12 text-center opacity-50 pointer-events-none">
              ...
              return (
                <div key={inv.id} className="bg-white rounded-2xl shadow-sm p-5">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    <button className="flex items-center gap-1.5 px-3 py-1.5 bg-rose-600 text-white rounded-lg text-xs font-medium"><Eye className="w-3.5 h-3.5" /> Voir</button>
                    <button className="flex items-center gap-1.5 px-3 py-1.5 border border-charcoal-200 text-charcoal-600 rounded-lg text-xs font-medium"><Download className="w-3.5 h-3.5" /> Télécharger</button>
                    <button className="flex items-center gap-1.5 px-3 py-1.5 border border-green-200 text-green-700 rounded-lg text-xs font-medium"><CheckCircle className="w-3.5 h-3.5" /> Paiement reçu</button>
                    <button className="flex items-center gap-1.5 px-3 py-1.5 border border-rose-200 text-rose-700 rounded-lg text-xs font-medium"><Send className="w-3.5 h-3.5" /> Envoyer au client</button>
                    <button className="flex items-center gap-1.5 px-3 py-1.5 border border-charcoal-200 text-charcoal-600 rounded-lg text-xs font-medium"><Edit className="w-3.5 h-3.5" /> Modifier</button>
                    <button className="flex items-center gap-1.5 px-3 py-1.5 border border-red-200 text-red-500 rounded-lg text-xs font-medium ml-auto"><Trash2 className="w-3.5 h-3.5" /></button>
                  </div>
                </div>
              );
            })}
