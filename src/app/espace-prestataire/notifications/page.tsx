        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <h1 className="font-serif text-charcoal-900" style={{ fontSize: 'clamp(1.4rem, 2.5vw, 1.8rem)', fontWeight: 400, letterSpacing: '-0.01em' }}>
              Notifications
              {unreadCount > 0 && (
                <span className="ml-2 text-sm font-sans font-semibold bg-rose-500 text-white px-2 py-0.5 rounded-full align-middle">
                  {unreadCount}
                </span>
              )}
            </h1>
          ...
          {notifications.length === 0 ? (
            ...
            <div className="divide-y divide-charcoal-50">
              {notifications.map((notif) => {
                ...
                const Icon = cfg.icon;
                const ts = notif.created_at?.toDate?.()?.toLocaleString('fr-FR')
                  || (notif.created_at ? new Date(notif.created_at).toLocaleString('fr-FR') : '');
                return (
                  <div
                    key={notif.id}
                    className={`flex items-start gap-4 p-4 transition-colors ${!notif.read ? 'bg-rose-50/60' : 'hover:bg-charcoal-50'}`}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <p className={`font-semibold text-sm ${!notif.read ? 'text-charcoal-900' : 'text-charcoal-600'}`}>
                          {notif.title}
                        </p>
