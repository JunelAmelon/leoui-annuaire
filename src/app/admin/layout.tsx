      ...
      >
        <div className="flex flex-col h-full bg-white rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className={`flex items-center pt-4 pb-3 flex-shrink-0 ${collapsed ? 'justify-center px-2' : 'px-4 gap-3'}`}>
            ...
            {!collapsed && (
              <div>
                <span className="font-serif text-charcoal-900 text-base leading-none truncate block">LeOui</span>
                <span className="text-[10px] text-charcoal-400 font-medium uppercase tracking-wider">Admin</span>
              </div>
            )}
          </div>
            ...
            <div className="relative group">
              <Link href="/admin/parametres"
                className={`flex items-center gap-3 rounded-xl transition-all ${
                  collapsed ? 'w-10 h-10 mx-auto justify-center' : 'px-3 py-2.5'
                } ${pathname === '/admin/parametres' ? 'bg-rose-600' : 'hover:bg-rose-50'}`}>
                <Settings className={`w-[17px] h-[17px] flex-shrink-0 ${pathname === '/admin/parametres' ? 'text-white' : 'text-charcoal-400 group-hover:text-rose-600'}`} />
                {!collapsed && <span className={`text-sm font-medium ${pathname === '/admin/parametres' ? 'text-white' : 'text-charcoal-600 group-hover:text-rose-700'}`}>Paramètres</span>}
              </Link>
            </div>
            ...
            <div className={`flex items-center gap-2.5 mt-1 ${collapsed ? 'justify-center' : 'px-3 py-1'}`}>
              <div className="w-8 h-8 rounded-full overflow-hidden shadow-sm flex-shrink-0">
                {(user as any)?.photoURL ? (
                  <img src={(user as any).photoURL} alt={displayName} className="w-full h-full object-cover" />
                ) : (
              ...
              {!collapsed && <span className="text-xs text-charcoal-500 truncate">{displayName}</span>}
            </div>
          ...
          <div className="absolute left-0 top-0 bottom-0 w-64 bg-white shadow-2xl flex flex-col z-50">
            <div className="px-5 py-4 border-b border-stone-100 flex items-center justify-between">
              <Link href="/" className="flex items-center gap-2.5" onClick={() => setMobileOpen(false)}>
                ...
                <span className="font-serif text-lg text-charcoal-900">LeOui Admin</span>
              </Link>
            ...
            <div className="border-t border-stone-100 py-2 px-2">
              <Link href="/admin/parametres" onClick={() => setMobileOpen(false)}
