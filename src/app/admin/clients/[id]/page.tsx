...
import { getClientDocuments, getClientVendors, calculateDaysRemaining } from '@/lib/client-helpers';
import { toast } from 'sonner';
import {
  ArrowLeft, Calendar, Users, Euro, MapPin, Phone, Mail, CheckCircle, FileText,
  MessageSquare, User, Pencil, Plus, X, Trash2,
} from 'lucide-react';
import Link from 'next/link';

type Tab = 'overview' | 'planning' | 'vendors' | 'documents' | 'messages';
...
export default function AdminClientDetailPage() {
  ...
  const fetchAll = async () => {
    try {
      const [clientData, eventDocs, vendorDocs, docDocs] = await Promise.all([
